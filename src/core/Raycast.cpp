	...
	std::function<bool(int, int, int)> isSolid)
{
	...
	for (int n = 0; n < maxSteps; ++n) {
		...
		if (isSolid(ix, iy, iz)) {
			...
			if (lastStepAxis == 0) result.normal = glm::vec3(-stepX, 0.f, 0.f);
			else if (lastStepAxis == 1) result.normal = glm::vec3(0.f, -stepY, 0.f);
			else if (lastStepAxis == 2) result.normal = glm::vec3(0.f, 0.f, -stepZ);
			else {
